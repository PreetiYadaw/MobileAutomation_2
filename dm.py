...
class DriverManager:
    ...
    def _start_web(self):
        ...
        if settings.chromedriver_path:
            # Bypass webdriver-manager's internet download entirely - use a
            # manually-downloaded chromedriver. Required on machines with a
            # restricted/no internet connection (corporate firewall, etc).
            service = ChromeService(settings.chromedriver_path)
        else:
            try:
                service = ChromeService(ChromeDriverManager().install())
            except Exception as e:
                raise RuntimeError(
                    "Could not auto-download chromedriver (this machine likely has "
                    "no/restricted internet access). Fix: download the chromedriver "
                    "matching your installed Chrome version from "
                    "https://googlechromelabs.github.io/chrome-for-testing/, unzip it "
                    "anywhere, and set CHROMEDRIVER_PATH in your .env to the full "
                    "path of chromedriver.exe. Original error: " + str(e)
                ) from e
